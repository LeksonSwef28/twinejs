...
- The post-#644 missing-history correction and negative test require a **new full exact-head CI run** after documentation closure. #644 must not be treated as proof of later source edits.
...
- After exact-head verification, recheck the stacked base, update PR description, perform final self-review and mark ready for review only if every check is green. **Merge is not authorized** without a separate immediate user instruction for each PR. Post-merge stable verification remains pending.
