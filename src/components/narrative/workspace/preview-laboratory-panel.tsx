	...
	return (
		<section className="preview-lab" aria-label="Preview authoring laboratory">
			<header className="preview-lab__header">
				<div>
					...
					<p className="simulation-debug__hint">
						...
						Force Outcome применяет только уже authored Outcome через настоящий runtime effect engine.
					</p>
				</div>
			...
			<div className="preview-lab__grid">
					...
					{active.project.claims.length > 0 && active.project.characters.length > 0 && (
						<>
							<div className="preview-lab__fields">
								<label>
									Knowledge character
									<select value={knowledgeCharacterId} onChange={event => setKnowledgeCharacterId(event.target.value)}>
										{active.project.characters.map(character => (
											<option key={character.id} value={character.id}>{character.name}</option>
										))}
									</select>
								...
								<label>
									...
									<select
										...
										onChange={event => setKnowledgeAttitude(event.target.value as KnowledgeAttitude)}
									>
								...
								<label>
									Confidence
									<input type="number" min={0} max={1} step={0.05} value={confidenceInput} onChange={event => setConfidenceInput(event.target.value)} />
								</label>
							</div>
							<div className="preview-lab__button-row">
								<button
									type="button"
									onClick={() => run(() => replaceActive(scenario => setPreviewRuntimeInput(scenario, {
										type: 'knowledge', characterId: knowledgeCharacterId, claimId,
										attitude: knowledgeAttitude, confidence: Number(confidenceInput)
									})))}
								>
									Set test knowledge
								</button>
								<button
									type="button"
									onClick={() => run(() => replaceActive(scenario => setPreviewRuntimeInput(scenario, {
										type: 'forget-claim', characterId: knowledgeCharacterId, claimId
									})))}
								>
									Forget in sandbox
								</button>
				...
				<section className="simulation-debug__card">
					<h3>Move eligibility / resolution</h3>
					<select value={selectedMoveId} onChange={event => setSelectedMoveId(event.target.value)}>
					...
					{selectedMove?.resolution.type === 'skill-check' && (
						<div className="preview-lab__fields">
							<label>
								Skill value
								<input type="number" value={skillValueInput} onChange={event => setSkillValueInput(event.target.value)} />
							</label>
							<label>
								Roll total
								<input type="number" value={rollTotalInput} onChange={event => setRollTotalInput(event.target.value)} />
							</label>
						</div>
					)}
					<div className="preview-lab__button-row">
						<button
							...
							onClick={() => run(() => setTrace(tracePreviewMove(active, selectedMoveId, resolutionInput())))}
						>
							Trace only
						</button>
						<button
							...
							disabled={!selectedMoveId}
							onClick={() => run(() => {
							const result = executePreviewMove(active, selectedMoveId, resolutionInput());
							setTrace(result.resolution);
							setLastOutcomeTrace(result.outcomeTrace);
							setScenarios(current => current.map(scenario => scenario.id === active.id ? result.scenario : scenario));
						})}
						>
					...
					<h4>Force authored Outcome · inspection only</h4>
				...
				<section className="simulation-debug__card">
					...
					<div className="preview-lab__button-row">
						{[1, 5, 30].map(minutes => (
							<button key={minutes} type="button" onClick={() => run(() => replaceActive(scenario => advancePreviewScenario(scenario, minutes)))}>
								+{minutes} мин
							</button>
						))}
					</div>
					...
					{changes?.changedPaths.length ? (
						<ul>{changes.changedPaths.slice(0, 50).map(path => <li key={path}><code>{path}</code></li>)}</ul>
					) : (
						<p className="simulation-debug__empty">Runtime не отличается от baseline.</p>
					)}
					{changes && <small>New occurrences: {changes.newOccurrenceIds.length} · lab actions: {changes.actionCount}</small>}
				</section>
				...
				<section className="simulation-debug__card">
					...
					{comparison ? (
						<>
							<p>{active.name} ↔ {compareScenario?.name}</p>
							{comparison.changedPaths.length ? (
								<ul>{comparison.changedPaths.slice(0, 50).map(path => <li key={path}><code>{path}</code></li>)}</ul>
							) : (
								<p className="simulation-debug__empty">Runtime states совпадают.</p>
							)}
						</>
					) : (
						<p className="simulation-debug__empty">Создайте fork и выберите сценарий для сравнения.</p>
					)}
				</section>
							...
							<ol>
								{active.actions.map(entry => (
									<li key={entry.id}>
										...
										{entry.occurrenceId && <code> occurrence {entry.occurrenceId}</code>}
									</li>
								))}
							</ol>
						</div>
							...
							{active.project.runtimeOccurrences.length ? (
								<ol>
									{active.project.runtimeOccurrences.slice(-20).map(occurrence => (
										<li key={occurrence.id}><code>{occurrence.id}</code> · {occurrence.type}</li>
									))}
								</ol>
